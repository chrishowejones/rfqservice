(ns rfqservice.core-test
  (:require [clojure.test :refer :all]
            [rfqservice.core :refer :all])
  (:import [java.util Optional]
           [rfqservice.core MyRfqService Quote]))

(deftest matching-quote
  (testing "Check quote for 200 USD returns expected values"
    (let [quote (quote-for (MyRfqService. 0.02M orders) :usd 200)]
  ...
  (testing "Check quote for 100 USD returns expected values"
    (let [quote (quote-for (MyRfqService. 0.02M orders) :usd 100)]
